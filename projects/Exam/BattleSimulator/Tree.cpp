...
Tree::Tree(int rows, int cols, float cellSize)
	: m_Rows{rows}, m_Cols{cols}, m_CellSize{cellSize}
{
	m_NodePointers.reserve(rows * cols);
	for (int r{}; r < rows; ++r)
	{
		for (int c{}; c < cols; ++c)
		{
			m_NodePointers.push_back(new Node{});
		}
	}
}
...
void Tree::Render() const
{
	...
	for (int r{}; r < m_Rows; ++r)
	{
		for (int c{}; c < m_Cols; ++c)
		{
			...
			DEBUGRENDERER2D->DrawString({ points[0].x + m_CellSize/2, points[0].y + m_CellSize/2}, std::to_string(m_NodePointers[GetCellId(r,c)]->GetAgentCount()).c_str());
		}
	}
}

void Tree::GetRowCol(int cellId, int& row, int& col) const
{
	row = cellId / m_Cols;
	col = cellId % m_Cols;
}

...
namespace Elite
{
	...
	void InfluenceMap<T_GraphType>::PropagateInfluence(float deltaTime)
	{
		...
		{
			//find highest influence neighbor
			float sourceInfluence{};
			float connectionCost{};
			for (auto& pConnection : GetNodeConnections(pNode))
			{
				auto pNeighbor = GetNode(pConnection->GetTo());

				float influence{ pNeighbor->GetInfluence() };
				//abs so also works with negatives
				if (abs(influence) > abs(sourceInfluence))
				{
